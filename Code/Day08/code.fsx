...
module Part2 =
    ...
    let calculate (source:int[,]) row col =
        [
            let cellValue = source[row, col]
            [for i in 0..col-1 do
                ("W", (if source[row, i] >= cellValue then 1 else 0), source[row, i])] |> List.rev
            [for i in col+1..lenCol-1 do
                ("E", (if source[row, i] >= cellValue then 1 else 0), source[row, i])]
            [for i in 0..row-1 do
                ("N", (if source[i, col] >= cellValue then 1 else 0), source[i, col])] |> List.rev 
            [for i in row+1..lenRow-1 do
                ("S", (if source[i, col] >= cellValue then 1 else 0), source[i, col])]
        ]
        |> List.concat
